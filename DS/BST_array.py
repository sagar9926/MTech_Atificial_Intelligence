...
def BST_search(bst_tree,k):
  ...
  for i in range(len(bst_tree)) :
    ...
    if bst_tree[i] is not None and hash_dict_val[bst_tree[i]] in bst_tree and bst_tree[i] != hash_dict_val[bst_tree[i]]:
      print("A = ",bst_tree[i]," and B = ",hash_dict_val[bst_tree[i]])
      flag = True
    elif not flag :
      return(-1)
